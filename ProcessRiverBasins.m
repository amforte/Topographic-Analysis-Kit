function ProcessRiverBasins(DEM,FD,S,river_mouths,varargin)
	...
	%			where the first column is a GRIDobj and the second column is a string identifying what this grid is (mostly so you can remember what these grids
	%			are when looking at outputs later). Any GRIDobj's provided must be identical to the input DEM in terms of size, resolution, and grid cell locations
	%			(e.g. if pre-processing your data in ArcGIS, you will want to use the DEM as the 'snap raster' when processing your extra grids to make sure that 
	%			the grids all overlie each other exactly).
	%
	% Examples:

function cmpBasinStatsPlots(wdir,BasinTableMat,plots,varargin)
	% Description:
	...
	% Parse Inputs
	p = inputParser;
	p.FunctionName = 'cmpBasinStatsPlots';
	addRequired(p,'wdir',@(x) ischar(x));
	addRequired(p,'BasinTableMat',@(x) regexp(x,regexptranslate('wildcard','*.mat')));
	...
	parse(p,wdir,BasinTableMat,plots,varargin{:});
	wdir=p.Results.wdir;
	TMAT=p.Results.BasinTableMat;
	...
	if save_figure
		...
		if num_figs>1
			for ii=1:num_figs
				fig_name=fullfile(wdir,['Figure_' num2str(ii) '.pdf']);
				orient(f(ii),'landscape');
				print(f(ii),'-dpdf','-fillpage',fig_name);
			end
		else % WEIRD SIMULINK ERROR FROM HELL
			fig_name=fullfile(wdir,'Figure_1.pdf');
			current=gcf;
			orient 'landscape';
			print(current,'-dpdf','-fillpage',fig_name);
		end

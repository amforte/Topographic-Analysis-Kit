...
\date{TAK Release Version 1.2.0}
...
\subsection{Compiled Functions (DEPRECATED)} \label{sec:CmpFunc}
\paragraph{}The original release of TAK included a set of companion compiled versions of the majority of TAK functions described in this manual. These required the MATLAB runtime environment to be installed and then were invoked at the command line, using either the 'Terminal' (MacOSX) or 'Command Prompt' (Windows) applications. While functional, these were ultimately hard for users to interact with and challenging for us to fully error test. For that reason, we are no longer updating or fixing these, though they do remain available (for now, but they will be removed from a future release) on the GitHub repository. We also maintain the description of their usage in the \nameref{sec:CmpDesc} section. These compiled conversions have been replaced with a GUI interface, described in the section \ref{sec:GUIgeneral} that can be run either within MATLAB or through the free MRE. 

\subsection{GUI Versions} \label{sec:GUIgeneral}
 \paragraph{}In TAK version 1.2.0 and later, we provide a GUI (Graphical User Interface) version of TAK. In detail, there are three versions of the GUI, a native MATLAB version (which does not require any additional installations, but does require all of the toolboxes, see \nameref{sec:Depend} for more details), and versions for both MacOSX and Windows which do not require MATLAB, but do require the free MATLAB Runtime Environment (MRE). If using the MRE version of the GUI, it is important that you install the MRE alongside TAK and not separately as this will ensure that you have installed the correct version of the MRE.

\begin{figure}[H]
	\includegraphics[width=16.5cm]{PNGs/TAK_GUI_DataInput}
	\caption{Opening screen of the TAK GUI.}
	\label{fig:TAKguiGeneral}
\end{figure}

\paragraph{}Within the GUI, most of the individual TAK functions appear in a variety of panels, grouped by the type of function, which roughly correspond to the sections in this manual. There are a few generic functions or operations that appear at the top of the GUI, specifically output controls, a function to dump the contents of a provided MAT file into individual files readable by standard GIS software, and some session controls (Figure \ref{fig:TAKguiGeneral}). 

\subsubsection{Installation}
\paragraph{MATLAB}To install the GUI for use in MATLAB, within the downloaded repository folder, navigate to the 'TAK\_mlapinstall' file within the 'GUI/MATLAB' folder from within MATLAB. Double click on this file and click on the 'Install' button in the dialog that appears. Now the TAK app is installed in your 'App' toolbar and can be run by clicking on the TAK icon.

\paragraph{Windows or Mac with Runtime} To install the GUI for use outside of MATLAB and using the Matlab Runtime Environment, navigate to the correct folder for your operating system (e.g. 'GUI/Windows/AppAndRuntime' or 'GUI/Mac/AppAndRuntime') and double click on the enclosed 'TAKAppInstaller\_web' file. This will download and install the necessary portion of the Matlab Runtime Environment and install TAK. TAK needs the specific version of the MRE that comes bundled with TAK, so it is usually best to install the MRE and TAK together, though the app without the MRE is provided in the 'JustTheApp' folder. After installation, the TAK application / executable that is created can be run by double clicking on it. It may take a few minutes (especially the first time) for the GUI to appear (and you will see a splash screen first), so be patient.

\subsubsection{General Use}
\paragraph{}Generally, any option that exists within the native MATLAB versions of the TAK functions appear as options in the GUI versions. With some limited exceptions, true-false options will appear as check boxes (where checked implies true and unchecked implies false), numeric parameters will be editable in dialog boxes (the few exceptions are when there are very specific ranges for input values), optional choices will appear as drop down menus, and definition of locations or loading of files is performed through buttons. All of the functions are setup such that the execution of the function will not occur until the \textbf{Run FunctionName} button is pushed. For some functions, upon clicking the run button, additional dialogs will appear to prompt the user to make additional choices or load extra data based on the users inputs. When running any function, there are a variety of required inputs, usually including selected portions of the datasets output from \textit{MakeStreams} and stored in a MAT file if created within the GUI along with an output directory in which to store any datasets that are generated. If you attempt to run a function without providing any or all of these required inputs, a series of warning dialogs will appear informing you which required inputs are missing (e.g. Figure \ref{fig:TAKguiWarn}).

\begin{figure}[H]
	\centering
	\includegraphics[width=8cm]{PNGs/TAK_GUI_warn}
	\caption{Example warning dialog when there is missing required data for a given function within the TAK GUI.}
	\label{fig:TAKguiWarn}
\end{figure}

\subsubsection{Output Controls}
\paragraph{}When using the GUI, you are required to provide a location in which to save outputs. This is set by clicking on the 'Select Output Directory' button and choosing a valid directory. If you attempt to run any function without providing a valid directory, you will be warned that you must. The path of this output directory will be displayed to the right of the button after you define it. The output controls also allow the user to change the default file name prefix, set to 'Topo' within the GUI. For the majority of functions, if an output is saved, the entry that appears in this box will be added to the front of the file name. 

\subsubsection{Session Controls} \label{sec:SessCntrls}
\paragraph{}There are a variety of options in the session controls panel, which appears in the top left of the GUI window (Figure \ref{fig:TAKguiGeneral}). For most of the TAK functions there are a variety of default values (i.e. the values that populate the dialog boxes upon starting a GUI session). If you ever want to reset all the values in these boxes to these default values without restarting the GUI, you can click on the 'Reset Defaults' button. As you interact with the GUI and use individual functions, you will end up loading a variety of files into active memory (e.g. DEM files, text files containing pour points, etc). If you want to clear active memory without restarting the GUI, you can click the 'Clear Loaded Data' button. This action will only clear loaded data and will not change the values of any parameters. Similarly, if you wish to save the current state of the GUI, you have two different options. 'Save Full State' will save the values for all the parameters along with any data you have loaded into the GUI. Alternatively, if you only want to save the values for parameters, you can use 'Save Parameter State'. Depending on the size of the datasets you have loaded in, saving the full state (and ultimately loading it back in) made be time consuming and produce a very large file, hence why the option to just save the state of the parameters is provided. The resultant MAT file produced from either option can be loaded back into the GUI with the 'Load Saved Data' button.

\subsubsection{Processing Status Indicator}
 \paragraph{}When the GUI is not actively processing data, the indicator light at the top right will be green and to the left of it, text saying 'Idle' will appear (Figure \ref{fig:TAKguiGeneral}). When a user command is being processed, the lamp will change to red and the text will change to 'Processing'. Giving the GUI additional commands while there are processes running may result in errors, so it is recommended that you do not click any additional buttons or change parameter values while the GUI is processing your previous commands.
 
 \subsubsection{Data Sharing within GUI}
 \paragraph{}Generally, the outputs of TAK functions run within the GUI will be available to other functions within that GUI session (or in a loaded session if the full state is saved). For example, if you run \textit{MakeStreams} within a GUI session, the majority of other functions will assume that you want to use the DEM, FLOWobj, flow accumulation GRIDobj, and STREAMobj created during this call of \textit{MakeStreams} as the primary input. You can always override this choice by using the 'Load MAT File' button that appears within most functions and providing a previously saved MAT file as output by the \textit{MakeStreams} function. If you're ever unsure of what files may be loaded into active memory, you can always use the 'Clear Loaded Data' button in the \nameref{sec:SessCntrls}. 
 
 \paragraph{}In addition to loaded data, some of the common parameters are shared between different functions, even those that appear in different panels of the GUI. For example, if you change the value of common parameters like the 'reference concavity' and 'threshold area' in one panel, these will be changed (and displayed) in the panels of other functions which also use this parameter. This is intentional to ensure consistency across a given analysis, but it can always be overridden, i.e. you can always change these values again when using a different function.
 
 \subsubsection{Data Outputs from the GUI}
 \paragraph{}Regardless of the version (i.e. the native MATLAB or one of the MRE versions), almost all functions will output a variety of files into the defined output directory. These will include files readable by standard GIS programs, but also a variety of MAT files. Within the context of the MRE versions of the GUI, the MAT files are mostly provided to serve as inputs for other TAK functions within the GUI, but many are also usable in terms of using the \textit{Mat2Arc} function to export files into formats readable by standard GIS programs. If you are using the GUI within MATLAB, outputs of functions will also be placed into the MATLAB workspace as if you were running the individual TAK functions from the command line.

\section{Error Reporting}
\paragraph{}We have tried to ensure that all options work properly within both the Matlab and GUI versions of TAK, but unexpected errors may occur. The preferred method of error reporting is to use the built in \href{https://github.com/amforte/Topographic-Analysis-Kit/issues}{'Issues'} function on the GitHub page for TAK so that there is a record of user encountered errors. If you have forked a version of the code and think you can fix an error, issue a pull request with the change, collaboration is welcome! If all else fails, you can email \href{mailto:aforte8@lsu.edu}{Adam Forte} as well. Before submitting an issue or contacting Adam, please check both open and closed issues to see if the bug has already been reported or fixed in an updated version. Whether submitting an issue or emailing Adam, please provide as much information as possible on the error (e.g. cut and paste the error message you received) and what you were doing when it occurred. For the GUI versions, errors may be significantly more opaque, i.e. the GUI may simply stop responding or quit. In these cases, please try to replicate the behavior to make sure it was not a one off glitch before reporting it as an error and if you are reporting it as an error, please try to be specific as possible as to what you were doing before the error occurred.

\section{Manual vs. Tutorial}
\paragraph{}You are reading the manual. This manual is designed to be a reference for the various TAK functions and focuses on explaining some of the details of how these functions work. It is \textbf{not} designed to be rigorous walk through of common usages of the functions. To fill that need, we have also provided a tutorial, which walks through an example dataset in some level of detail showing how the majority of the TAK functions can be used. 

\paragraph{}Two different versions of the tutorial are provided, one focused on using the native MATLAB functions and one focused on using the GUI. The example dataset for the two tutorials are the same. Both tutorials and the example data set are available in releases of TAK after v1.2. Please note that the 'ExampleData.tar.gz' is the example dataset referenced in this manual. The 'TutorialFiles.tar.gz' included with the release contains the example dataset for the tutorial (along with the tutorial presentations). 
...
\noindent
TAK requires licenses for the Image Processing Toolbox, Mapping Toolbox, Optimization Toolbox, and Statistics and Machine Learning Toolbox. If you do not have all licenses for all of these, you may need to use the MRE version of the GUI.
...
\subsection{\textit{ConditionDEM}} \label{sec:ConDem}
...
\subsection{\textit{RemoveFlats}} \label{sec:RemFlats}
...
\subsection{GUI Data Input}
\paragraph{}The four preceding tools, \nameref{sec:MakeStreams}, \nameref{sec:ConDem}, \nameref{sec:RemFlats}, and \nameref{sec:FindThresh}, all appear in the 'Data Input' tab of the TAK GUI (Figure \ref{fig:TAKguiGeneral}).
...
\subsection{\textit{SegmentPlotter}} \label{sec:SegPlot}
...
\subsection{\textit{ProjectedIncision}} \label{sec:ProjInc}
...
\subsection{GUI Stream Selection and Projection}
\paragraph{}The four preceding functions, \nameref{sec:SegPick}, \nameref{sec:SegPlot}, \nameref{sec:SegProj}, and \nameref{sec:ProjInc}, appear in the 'Stream Selection and Projection' tab of the GUI (Figure \ref{fig:TAKguiProj}). There are several options that are shared between all, or most, of these functions, so to avoid clutter in the GUI, there are only single entry boxes for those parameters, e.g. the reference concavity.

\begin{figure}[H]
	\includegraphics[width=16.5cm]{PNGs/TAK_GUI_Proj}
	\caption{The stream selection and projection tab within the TAK GUI.}
	\label{fig:TAKguiProj}
\end{figure}
...
\subsection{\textit{ClassifyKnicks}} \label{sec:ClassKnicks}
...
\subsection{GUI Ksn \& Chi Maps}
\paragraph{}Four out of the five preceding functions, \nameref{sec:KsnBatch}, \nameref{sec:KsnAuto}, \nameref{sec:KsnProfiler}, and \nameref{sec:ClassKnicks}, appear in the 'Ksn \& Chi Maps' tab of the GUI (Figure \ref{fig:TAKguiKsnChi}). There are several options that are shared between all, or most, of these functions, so to avoid clutter in the GUI, there are only single entry boxes for those parameters, e.g. the reference concavity. This panel of the GUI also includes options to plot the result of producing maps of either k\textsubscript{sn} or $\chi$.

\begin{figure}[H]
	\includegraphics[width=16.5cm]{PNGs/TAK_GUI_KsnChi}
	\caption{The k\textsubscript{sn} and $\chi$ map tab within the TAK GUI.}
	\label{fig:TAKguiKsnChi}
\end{figure}
...
\subsection{GUI Junctions and Erosion}
\paragraph{}The three preceding functions, \nameref{sec:JAngle}, \nameref{sec:JLinks}, and \nameref{sec:InJunc}, along with the \nameref{sec:EroGrid} function appear in the 'Junctions and Erosion' tab of the GUI (Figure \ref{fig:TAKguiJuncs}).

\begin{figure}[H]
	\includegraphics[width=16.5cm]{PNGs/TAK_GUI_Juncs}
	\caption{The Junctions and Erosion tab within the TAK GUI.}
	\label{fig:TAKguiJuncs}
\end{figure}
...
\paragraph{Note for GUI Versions:} If you are running the GUI version of \textit{ProcessRiverBasins}, the procedure for adding extra grids is quite easy. Just click on the 'Add Raster Grids' button and navigate to the location of a suitable raster (saved as either a GeoTiff or ASCII in the same coordinate system as the original DEM). The GUI will then prompt you to provide a name for this raster (this is equivalent to the second input of the cell array in the command line usage). If you wish to add more than one raster, simply click the button again, the GUI will display a running list of the number of rasters you've loaded as 'Additional Grids'.
...
\paragraph{Note for GUI Versions:} If you are running the GUI version of \textit{ProcessRiverBasins}, the procedure for adding categorical grids is quite easy and similar to adding raster grids. Just click on the 'Add Categorcial Grids' button and navigate to the location of a suitable polygon shapefile. The GUI will then prompt you to select which field of this shapefile you wish to use (equivalent to the second input to \textit{CatPoly2GRIDobj}) and a name to give this categorical grid (equivalent to the third input to the cell array in the command line usage). If you wish to add more than one categorical grid, simply click the button again, the GUI will display a running list of the number of categorical grids you've loaded as 'Categorical Grids'. 
...
\paragraph{Note for GUI Versions:} There is no compiled version of \textit{CatPoly2GRIDobj} as this is bound up within the GUI version of \nameref{sec:PrcRvBsn}.
...
\paragraph{Note for GUI Versions:} In the GUI version of \textit{Basin2Shape}, extra field values are dealt with differently. Instead of providing separate inputs for the values and names for those values, a text file is loaded with the 'Load Extra Values' and the names of columns in that text file are used as the names of the resulting categories. 

\subsection{\textit{Basin2Raster}} \label{sec:B2R}
...
\paragraph{Note for GUI Versions:} If using the GUI version of \textit{CompileBasinStats} and you wish to supply extra fields, the procedure is identical as described for the GUI version of \nameref{sec:B2S}.
...
\subsection{GUI Select, Extract, and Summarize  Basins}
\paragraph{}The functions associated with selecting, extracting, and summarize basins are split between two tabs in the TAK GUI, the 'Select and Extract Basins' tab (Figure \ref{fig:TAKguiExBasins}) and the 'Summarize Basins' (Figure \ref{fig:TAKguiSumBasins}) tabs. The \nameref{sec:BsnPckr}, \nameref{sec:PrcRvBsn}, \nameref{sec:SbDvdBsn}, and \nameref{sec:BsnKncks} functions appear in the 'Select and Extract Basins' tab. The \nameref{sec:B2R}, \nameref{sec:B2S}, \nameref{sec:CompStats}, and \nameref{sec:BSPlots} functions appear in the 'Summarize Basins' tab. The \nameref{sec:CatGrid} function does not appear in the GUI, but its functionality is embedded within the \textit{ProcessRiverBasins} function and is invoked if the 'Add Categorical Grids' button is pushed. 

\begin{figure}[H]
	\includegraphics[width=16.5cm]{PNGs/TAK_GUI_ExBasins}
	\caption{The Select and Extract Basins tab within the TAK GUI.}
	\label{fig:TAKguiExBasins}
\end{figure}

\begin{figure}[H]
	\includegraphics[width=16.5cm]{PNGs/TAK_GUI_SumBasins}
	\caption{The Summarize Basins tab within the TAK GUI.}
	\label{fig:TAKguiSumBasins}
\end{figure}
...
\subsection{\textit{ProjectSmallCircleOntoSwath}} \label{sec:ProjCircSwath}
...
\subsection{\textit{ProjectGPSOntoSwath}} \label{sec:ProjGPSSwath}
...
\subsection{GUI Swath}
\paragraph{}The \nameref{sec:Swath}, \nameref{sec:CombSwath}, and \nameref{sec:SerSwath} functions appear in the 'Swath' tab of the TAK GUI (Figure \ref{fig:TAKguiSwath}). The version of \textit{MakeCombinedSwath} implemented in the GUI can be run on a single swath or on the output of \textit{MakeSerialSwath}. The \nameref{sec:ProjSwath}, \nameref{sec:ProjCircSwath}, and \nameref{sec:ProjGPSSwath} functions are not formally included in the GUI versions, but are the outputs of these functions are accessible through the provided functions. E.g. the data that is output from running \textit{MakeCombinedSwath} will include the projected form of the provided data, which will be the product of \textit{ProjectOntoSwath}, \textit{ProjectSmallCircleOntoSwath}, \textit{ProjectGPSOntoSwath} depending on the exact choices. 

\begin{figure}[H]
	\includegraphics[width=16.5cm]{PNGs/TAK_GUI_Swath}
	\caption{The Swaths tab within the TAK GUI.}
	\label{fig:TAKguiSwath}
\end{figure}
...
%%%%%%%%%%%%%%%%%%%%%
\section{Original Description of Compiled Functions} \label{sec:CmpDesc}
\paragraph{}It is recommended you use the MRE GUI version of TAK as opposed to the old compiled functions as these are no longer maintained and will be removed from a future release. We include the original description of these for reference.
...
\section{Headers for Original Compiled Functions}\label{sec:CmpFuncCalls}

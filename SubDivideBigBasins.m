function SubDivideBigBasins(location_of_data_files,max_basin_size,divide_method,varargin)
	...
	%		min_basins_size [10] - minimum basin size in km^2 for auto-selecting sub basins used if 'divide_method' is 'filtered_confluences'
	...
	addRequired(p,'divide_method',@(x) ischar(validatestring(x,{'order','confluences','up_confluences','filtered_confluences'})));
	...
	for ii=1:num_files;
		...
		if strcmp(clip_method,'clip')
			load(FileName,'RiverMouth','drainage_area','DEMoc','FDc','Ac','Sc','ksn_method');
			DEM=DEMoc;
			S=Sc;
			FD=FDc;
			A=Ac;
		...
		if DA>=max_basin_size
			...
			case 'filtered_confluences'
				...
				if strcmp(clip_method,'clip')
					DAG=A.*(A.cellsize^2);
				elseif strcmp(clip_method,'segmetn')
			...
			case 'clip'
				for jj=1:num_new_basins
					...
					if ~isempty(VarInd)
						...
						save(SubFileName,'RiverMouth','DEMcc','DEMoc','out_el','drainage_area','FDc','Ac','Sc','SLc','Chic','Goc','MSc','MSNc','KSNc_stats','Gc_stats','Zc_stats','Centroid','ChiOBJc','ksn_method','clip_method','AGc','AGc_stats');
			...
			case 'segment'
				for jj=1:num_new_basins
					...
					if ~isempty(VarInd)
						...
						save(SubFileName,'RiverMouth','DEMoc','out_el','drainage_area','Sc','SAc','Goc','MSc','MSNc','KSNc_stats','Gc_stats','Zc_stats','Centroid','ChiOBJc','ksn_method','clip_method','AGc','AGc_stats');
					elseif isempty(VarInd)
